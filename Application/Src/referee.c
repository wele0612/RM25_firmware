...
static referee_transmit_fn_t transmit_fn = NULL;

void referee_init(referee_transmit_fn_t tx_fn){
    transmit_fn = tx_fn;
}

static float corner_x[4], corner_y[4]; // To show base pose
...
void referee_reset_ui(){
    ...
    // draw_rectangle(&(ui_data.user_data.seven_graphics[1]), "ci", FIGURE_OPERATION_ADD, 1,
    //     COLOR_PINK, 6, 181, 795, 182+32, 795+32);
    
    // draw_arc(&(ui_data.user_data.seven_graphics[2]), "cp", FIGURE_OPERATION_ADD, 1, 
    //     COLOR_YELLOW, 8, 960, 536, 220, 320, 340, 350);

    // draw_line(&(ui_data.user_data.seven_graphics[4]), "bl1", FIGURE_OPERATION_ADD, 1,
    //     COLOR_CYAN, 3, (uint16_t)corner_x[0], (uint16_t)corner_y[0], (uint16_t)corner_x[1], (uint16_t)corner_y[1]);

    // draw_line(&(ui_data.user_data.seven_graphics[5]), "bl2", FIGURE_OPERATION_ADD, 1,
    //     COLOR_CYAN, 3, (uint16_t)corner_x[2], (uint16_t)corner_y[2], (uint16_t)corner_x[3], (uint16_t)corner_y[3]);

    // draw_arc(&(ui_data.user_data.seven_graphics[6]), "bl3", FIGURE_OPERATION_ADD, 1,
    //     COLOR_ORANGE, 1, 200, 200, 10, 15, 20, 20);

    // float content_length = sizeof(interaction_figure_t)*7;
    // uint8_t *send_buf = referee_send_data(6 + content_length, &ui_data);
    // transmit_fn(send_buf, 15+content_length);

    // const int x_offset = 0;
    // const int y_offset = 0;

    // //横线
    // draw_line(&(ui_data.user_data.seven_graphics[0]), "z0", FIGURE_OPERATION_ADD, 2,
    //     COLOR_ORANGE, 2, 960 - 20 + x_offset, 516 + y_offset,
    //         960 + 20 + x_offset, 516 + y_offset);
    // draw_line(&(ui_data.user_data.seven_graphics[1]), "z1", FIGURE_OPERATION_ADD, 2,
    //     COLOR_ORANGE, 2, 960 - 20 + x_offset, 486 + y_offset,
    //         960 + 20 + x_offset, 486 + y_offset);
    // draw_line(&(ui_data.user_data.seven_graphics[2]), "z2", FIGURE_OPERATION_ADD, 2,
    //     COLOR_ORANGE, 2, 960 - 35 + x_offset, 536 + y_offset,

    // //竖线
    //         960 + 35 + x_offset, 536 + y_offset);
    // draw_line(&(ui_data.user_data.seven_graphics[3]), "z3", FIGURE_OPERATION_ADD, 2,
    //     COLOR_ORANGE, 2, 960 + x_offset, 536 + 30 + y_offset,
    //         960 + x_offset, 536 - 80 + y_offset);
    ...
    // strcpy(ui_data.user_data.char_graphic.char_data, helloworld);
    // draw_char(&(ui_data.user_data.char_graphic.graphic_data), "ct1", FIGURE_OPERATION_ADD, 3,
    ...
    // strcpy(ui_data.user_data.char_graphic.char_data, str2);
    // draw_char(&(ui_data.user_data.char_graphic.graphic_data), "ct2", FIGURE_OPERATION_ADD, 3,
    ...
    // strcpy(ui_data.user_data.char_graphic.char_data, str3);
    // draw_char(&(ui_data.user_data.char_graphic.graphic_data), "ct3", FIGURE_OPERATION_ADD, 3,
...
void referee_ui_next_frame(){
    ...
    // const uint16_t cap_percentage = capcan_cap2b.cap_energy_percentage;
    // draw_arc(&(ui_data.user_data.seven_graphics[2]), "cp", FIGURE_OPERATION_MODIFY, 1, 
    //     COLOR_YELLOW, 8, 960, 536, 218, 222+cap_percentage, 340, 350);

    // int ctrl_opt = -1;
    // if (dr16.key.v & DR16_KEY_CTRL_BIT){
    ...
    //     }
    // }    
    // float ctrl_y_offset = 30*ctrl_opt;
    // draw_rectangle(&(ui_data.user_data.seven_graphics[3]), "ci", FIGURE_OPERATION_MODIFY, 1,
    //     COLOR_PINK, 6, 181, 795 - ctrl_y_offset, 182+32, 795+32 - ctrl_y_offset);

    // float base_rec_cx = 960.0f, base_rec_cy = 100.0f;  // 中心坐标
    // const float base_rec_width = 60, base_rec_height = 150; // 尺寸
    // //float base_rec_rot = 0.2f*((float)HAL_GetTick() * 0.001f);
    // float base_rec_rot = 0.2f;

    // const float half_w = base_rec_width * 0.5f;
    // const float half_h = base_rec_height * 0.5f;
    // const float sin_r = sinf(base_rec_rot);  // 预计算
    // const float cos_r = cosf(base_rec_rot);

    // // 右上角 [0]
    // corner_x[0] =  half_w * cos_r - half_h * sin_r + base_rec_cx;
    // corner_y[0] =  half_w * sin_r + half_h * cos_r + base_rec_cy;

    // // 右下角 [1]
    // corner_x[1] =  half_w * cos_r + half_h * sin_r + base_rec_cx;
    // corner_y[1] =  half_w * sin_r - half_h * cos_r + base_rec_cy;

    // // 左下角 [2]
    // corner_x[2] = -half_w * cos_r + half_h * sin_r + base_rec_cx;
    // corner_y[2] = -half_w * sin_r - half_h * cos_r + base_rec_cy;

    // // 左上角 [3]
    // corner_x[3] = -half_w * cos_r - half_h * sin_r + base_rec_cx;
    // corner_y[3] = -half_w * sin_r + half_h * cos_r + base_rec_cy;

    // draw_line(&(ui_data.user_data.seven_graphics[4]), "bl1", FIGURE_OPERATION_MODIFY, 1,
    //     COLOR_CYAN, 3, (uint16_t)corner_x[0], (uint16_t)corner_y[0], (uint16_t)corner_x[1], (uint16_t)corner_y[1]);

    // draw_line(&(ui_data.user_data.seven_graphics[5]), "bl2", FIGURE_OPERATION_MODIFY, 1,
    //     COLOR_CYAN, 3, (uint16_t)corner_x[2], (uint16_t)corner_y[2], (uint16_t)corner_x[3], (uint16_t)corner_y[3]);

    // const float rad_2_deg = (180.0f/3.1415927f);
    // const float base_rec_rot_deg = base_rec_rot * rad_2_deg;
    // draw_arc(&(ui_data.user_data.seven_graphics[6]), "bl3", FIGURE_OPERATION_MODIFY, 1,
    //     COLOR_ORANGE, 6, (uint16_t)base_rec_cx, (uint16_t)base_rec_cy, base_rec_rot_deg - 25, base_rec_rot_deg + 25,
    //     base_rec_height, base_rec_height);

    // content_length = sizeof(interaction_figure_t)*7;
    // uint8_t *send_buf = referee_send_data(6 + content_length, &ui_data);
    // transmit_fn(send_buf, 15+content_length);

    //Important! The UI server cannot receive frames too much
    ...
    // strcpy(ui_data.user_data.char_graphic.char_data, helloworld);
    // draw_char(&(ui_data.user_data.char_graphic.graphic_data), "ch", FIGURE_OPERATION_ADD, 2,
    //     COLOR_ORANGE, 5, 700, 350, 30, 13);

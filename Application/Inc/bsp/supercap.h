/**
 * @file cap_canmsg_protocal.h
 * @author The Great Lakes
 * @brief The protocal defination for super capacitor controller.
 * @version 1.0
 * @date 2024-07-29
 * 
 * @copyright Copyright (c) 2024
 * 
 */
#ifndef __SUPERCAP_H
#define __SUPERCAP_H

#include <stdint.h>

/* Message send to capacitor module */
#define CAPCAN_TOCAP_MSG_ID 0x2C8  //CAN ID to supercap

//ALL UNITS IN watt, multiplies by 100

typedef struct capcan_rx_t{
    uint16_t power_target;  // 1e-2 W/LSB
    uint16_t referee_power; // 1e-2 W/LSB
    uint16_t rsvd1; //Must be 0x2012
    uint16_t rsvd2; //Must be 0x0712
}capcan_toCap_t;


/*Message come from capacitor module */
/*Expected message frequency = 100Hz */
#define CAPCAN_FROMCAP_MSG_ID 0x2C7  //CAN ID from Supercap

typedef struct capcan_tx_t{
    uint16_t max_discharge_power; // 1e-2 W/LSB
    int16_t base_power;          // 1e-2 W/LSB
    int16_t cap_energy_percentage;
    uint16_t cap_state;
}capcan_fromCap_t;

enum Cap_states{
    CAP_OFF,
    CAP_READY,
    CAP_ON,
    VBUS_OVP,
    VBUS_UVP,
    VBAT_OVP,
};

#endif

void supercap_update_timeout();

int supercap_online();
